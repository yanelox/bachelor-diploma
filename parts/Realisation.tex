...
было обнаружено, что для кортежных типов, таких как vint8m4x2\_t, компилятор LLVM
...
было реализовать корректную передачу аргументов в функцию перед ее вызовом. Вторым требованием
...
Например, тип \_\_rvv\_int8m1\_t (vint8m1\_t).
...
В данной функции мы сначала определяем параметры LMUL и NFIELDS из названия типа,
для того, чтобы определить количество необходимых нам регистров. Далее, вызывая
